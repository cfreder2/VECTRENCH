...
| `boss` | top level, not a section, and **optional**: a warden waiting past the port. `{ "kind", "name", "hp" (20–400), "weapon", "weakTo", "arena" }`. With a boss, killing the port turns the flyout into an entrance — the track grows a flat 6000-unit arena past the runout, the music slams to the boss theme, and the fight is flown on open ground until one of you is done. Without it the level ends exactly as it always has, which is what keeps every track usable for plain timed runs. `arena` is `"surface"` (default; `"trench"` and `"void"` normalize but are reserved). The warden fights with its `weapon`, half-resists it, and takes triple from `weakTo` |
