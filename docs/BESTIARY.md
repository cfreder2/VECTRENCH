...
| **Roll** | A double tap with the second tap held down — there is no roll button, because a button in a fixed corner cannot say *which way*. 4.2 rad/s onto its edge, trading 14×10 for 5×18 — width for height. **Signed**: it goes over to whichever side you asked for, because rolling only ever one way is no use when the thing you are avoiding is on that side |
