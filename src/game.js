...
const DIAMOND_BASE = 3;       // a run starts here, and the recharge stops here
const DIAMOND_CAP = 8;        // gates can stack it to here
const DIAMOND_RECHARGE = 20;  // seconds from bone dry back to base
...
export class Game {
  ...
  updateShip(dt) {
    ...
    // The special is held, like the burn: down is firing, up is saved -- and
    // the meter is a tank, not a switch. It drains for exactly as long as the
    // button is down, keeps exactly what is left when it comes up, and from
    // bone dry recharges back to base in twenty seconds. Draining it dry
    // latches it off until the button is released, so a held button does not
    // sputter as the recharge drips in.
    ...
    if (this.specialOn) {
      ...
      this.diamonds = Math.min(DIAMOND_BASE,
        this.diamonds + dt * (DIAMOND_BASE / DIAMOND_RECHARGE));
    }
  ...
  sayGuard() {
    ...
    this.say((this.boss && this.boss.guardMsg) || 'ITS ARMOR HOLDS', 1.4);
  }
